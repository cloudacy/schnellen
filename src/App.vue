...
const newPlayerInput = ref<{input: HTMLInputElement | undefined}>()
...
const addPlayer = () => {
  ...
  newPlayerInput.value?.input?.focus()
}
  ...
  <template v-if="step === 1">
    ...
    <form class="mb-3" @submit.prevent="addPlayer">
      ...
      <div class="d-flex align-items-flex-end mb-3">
        <TextInput id="new-player-input" ref="newPlayerInput" label="Spieler" icon="users" placeholder="John" :autofocus="true" v-model="newPlayerName" required />
        <div class="ml-3 mb-2">
    ...
    <button class="btn btn-primary" :disabled="startPoints < 1 || Object.keys(history).map(k => history[k]).length < 1" @click.prevent="startGame">
        ...
        <tfoot>
          <tr>
            <td v-for="(v, p) in round" :key="p">
              ...
              <div v-else>Raus</div>
              <div class="btn-group">
                <button class="btn btn-primary btn-small btn-icon" :disabled="v !== null && (v >= 5 || sumPoints >= 5)" @click.prevent="addPoint(p)">
                  <font-awesome-icon class="icon" icon="plus" />
                </button>
                <button class="btn btn-primary btn-small btn-icon" v-if="v !== null && v > 0" @click.prevent="subPoint(p)">
